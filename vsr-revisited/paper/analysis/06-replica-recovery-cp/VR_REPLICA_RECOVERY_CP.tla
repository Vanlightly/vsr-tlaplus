...
This specification adds replica recovery with checkpointing.
...
- State Transfer
- Application state
- Replica recovery with the checkpoint based optimization
...
CONSTANTS ReplicaCount,             \* the number of replicas in the cluster
          ...
          CrashLimit                \* state space limiter, number of crashes permitted (keep to minority)
...
CONSTANTS Nil,
          NoOp,   \* used to signal part of the GCed log prefix
...
DoViewChangeMsgType ==
    [type: DoViewChangeMsg,
     view_number: Nat,
     log_suffix: [Nat -> LogEntryType],
     checkpoint: [Nat -> LogEntryType],
     cp_number: Nat,
     last_normal_vn: Nat,
     op_number: Nat,
     commit_number: Nat,
...
GetCheckpointMsgType ==
    [type: GetCheckpointMsg,
     dest: {AnyDest},
     source: Nat]
         
NewCheckpointMsgType ==
    [type: NewCheckpointMsg,
     cp_number: Nat,
     checkpoint: [Nat -> LogEntryType],
     dest: Nat,
     source: Nat]
...
RecoveryResponseMsgLogSyncType ==
...
RecoveryResponseMsgCheckpointSyncType ==
    [type: RecoveryResponseMsg,
     view_number: Nat,
     x: Nat,
     flag: {1},
...
HighestGCedOp(log) ==
    CASE Len(log) = 0 -> 0
      [] ~\E op \in 1..Len(log) : log[op] = NoLogEntry -> 0
      [] OTHER ->
            CHOOSE op \in 1..Len(log) :
                /\ log[op] = NoLogEntry
                /\ ~\E op1 \in 1..Len(log) :
                    /\ log[op1] = NoLogEntry
                    /\ op1 > op
...
EmptyLog(op_number) ==
    [op \in 1..op_number |-> NoLogEntry]   
   
LogPrefix(r, log, op_number) ==
    IF op_number = 0
    THEN <<>>
    ELSE [op \in 1..op_number |-> log[op]]
...
RECURSIVE ExecuteOps(_,_,_,_)
ExecuteOps(log, app_state, op, commit_number) ==
    IF op > commit_number
    THEN app_state
    ELSE LET app_state1 == Append(app_state, log[op])
         IN ExecuteOps(log, app_state1, op+1, commit_number)

MaybeExecuteOps(r, log, old_commit, new_commit) ==
    IF new_commit > old_commit
    THEN /\ rep_app_state' = [rep_app_state EXCEPT ![r] = 
                                    ExecuteOps(log, @, old_commit + 1, new_commit)]
         /\ rep_commit_number' = [rep_commit_number EXCEPT ![r] = new_commit]
    ELSE UNCHANGED << rep_app_state, rep_commit_number >>

Checkpoint(r, cp) ==
    LogPrefix(r, rep_app_state[r], cp)

ApplyNewState(log, app_state, new_commit) ==
    LET curr_commit == MaxOpNumber(app_state)
    IN IF new_commit > curr_commit
       THEN ExecuteOps(log, app_state, curr_commit+1, new_commit)
       ELSE app_state
       
ApplyCheckpoint(r, log_suffix, checkpoint, cp_number, op_number, new_commit) ==
    LET log == [op \in 1..op_number |->
                            IF op <= cp_number
                            THEN NoLogEntry        \* remove replica log prefix covered by checkpoint
                            ELSE log_suffix[op]] \* take ops from message log suffix above checkpoint
    IN /\ rep_log' = [rep_log EXCEPT ![r] = log]
       /\ rep_app_state' = [rep_app_state EXCEPT ![r] = 
                                 ApplyNewState(log_suffix, checkpoint, new_commit)]
       /\ rep_op_number' = [rep_op_number EXCEPT ![r] = op_number]
       /\ rep_commit_number' = [rep_commit_number EXCEPT ![r] = new_commit]

    
ReplyWithCheckpointAndLogSuffix(r, recv_msg, send_msg) ==
    \E last_checkpoint_number \in HighestGCedOp(rep_log[r])+1..rep_commit_number[r] :
        LET cp_fields ==              
                [flag          |-> 1,
                 cp_number     |-> last_checkpoint_number,
                 checkpoint    |-> LogPrefix(r, rep_app_state[r], last_checkpoint_number),
                 log_suffix    |-> LogSuffix(r, rep_log[r], last_checkpoint_number+1),
                 commit_number |-> last_checkpoint_number]
            msg      == send_msg @@ cp_fields
        IN DiscardAndSend(recv_msg, msg)

ReplyWithLogSuffix(r, recv_msg, send_msg) ==
    LET ls_fields == [flag          |-> 0,
                      log_suffix    |-> LogSuffix(r, rep_log[r], recv_msg.op_number+1),
                      first_op      |-> recv_msg.op_number + 1,
                      commit_number |-> rep_commit_number[r]]
        msg       == send_msg @@ ls_fields
    IN DiscardAndSend(recv_msg, msg)
    
ReplyWithNilLogSuffix(r, recv_msg, send_msg) ==
    LET ls_fields == [flag       |-> 0,
                      log_suffix |-> Nil,
                      first_op   |-> Nil]
        msg       == send_msg @@ ls_fields
...
\* ExecuteOp
\*
\* The replica executes an op in order.
\* In this spec executing an op is basically a no-op and appending
\* the op to the app_state which is a log. 
\* It also increments the commit number. 
...
\* ReceiveGetState
\*
\* A replica receives a GetState message with a matching view.
\* If the replica will either send a log suffix or a checkpoint
\* and log suffix. 
\* If the replica has the portion of the log required, then it
\* uses log based synchronization by sending the log suffix.
\* Else if part of the log suffix required is in the garbage collected
\* prefix then it uses app state synchronizations by sending its latest
\* checkpoint and log suffix that is not covered by the checkpoint.
\* It ignores GetState messages with an op number that is
\* higher or equal to its own (this is an optimzation
\* for the spec to avoid the need for retries, an implementation would 
\* need to take into account the situation where the message op-number is
...
ReceiveGetState ==
    \E r \in replicas, m \in DOMAIN messages :
        ...
        \* mutations to state
        /\ LET msg == [type          |-> NewStateMsg,
                       view_number   |-> View(r),
                       op_number     |-> rep_op_number[r],
                       dest          |-> m.source,
                       source        |-> r]
           IN IF rep_log[r][m.op_number+1] = NoLogEntry
              THEN ReplyWithCheckpointAndLogSuffix(r, m, msg) 
              ELSE ReplyWithLogSuffix(r, m, msg)
...
\* the StateTransfer state. The NewState may take
\* two forms:
\*  - If the NewState message only contains a log suffix,
\*    it overwrites/appends the log entries to its log 
\*    and executes any operations if the received commit-number
\*    was higher than its own.
\*  - If the NewState message contains a checkpoint it 
\*    writes the checkpoint to its app state and overwrites
\*    /appends the log entries to its log. It executes any 
\*    operations if the received commit-number was higher than
\*    the cp-number.
\* It also updates its view and last normal view and 
\* switches back to Normal status.
...
ReceiveNewState ==
    \E r \in replicas, m \in DOMAIN messages :
        ...
        /\ IF m.flag = 0
           THEN LET log == [op \in 1..m.op_number |->
                                IF op < m.first_op
                                THEN rep_log[r][op]    \* keep operations in replica log prefix
                                ELSE m.log_suffix[op]] \* use operations in message log suffix
                IN /\ rep_log' = [rep_log EXCEPT ![r] = log]
                   /\ MaybeExecuteOps(r, log, rep_commit_number[r], m.commit_number)
                   /\ rep_op_number' = [rep_op_number EXCEPT ![r] = m.op_number]
           ELSE ApplyCheckpoint(r, m.log_suffix, m.checkpoint, 
                                m.cp_number, m.op_number, m.commit_number)
...
\* - it's latest checkpoint (if any) and its cp number.
\* - it's log suffix not covered by its checkpoint
...
SendDVC ==
    \E r \in replicas :
        ...
        /\ \E last_cp \in HighestGCedOp(rep_log[r])+1..rep_commit_number[r] :
           LET msg == [type           |-> DoViewChangeMsg,
                       view_number    |-> View(r),
                       log_suffix     |-> LogSuffix(r, rep_log[r], last_cp+1),
                       checkpoint     |-> Checkpoint(r, last_cp),
                       cp_number      |-> last_cp,
...
\* - from the winning DVC:
\*    - the checkpoint and cp number
\*    - the log suffix
\*    - the op number
\* - the highest commit-number of all DVCs received
\* It also overwrites its own app state and log with 
\* the winning checkpoint and log suffix, and executes 
\* any operations between the cp-number and the new
\* commit-number. This is not optimum for an implementation
\* but is as close to sending the full log as is possible
...
SendSV ==
    \E r \in replicas :
        ...
        \* mutations to state
        /\ LET winning_dvc == WinningDVC(r)
               new_cn      == HighestCommitNumber(r)
           IN
                /\ rep_status' = [rep_status EXCEPT ![r] = Normal]
                /\ ApplyCheckpoint(r, winning_dvc.log_suffix, winning_dvc.checkpoint,
                                   winning_dvc.cp_number, winning_dvc.op_number, new_cn)
                ...
                /\ Broadcast([type          |-> StartViewMsg,
                              view_number   |-> View(r),
                              log_suffix    |-> winning_dvc.log_suffix,
                              checkpoint    |-> winning_dvc.checkpoint,
                              cp_number     |-> winning_dvc.cp_number,
                              op_number     |-> winning_dvc.op_number,
...
\* ReceiveSV (StartView)
\*
\* A replica receives a StartView message and applies
\* the received checkpoint and log suffix. If the replica
\* has any uncommitted operations in its new log below
\* the new commit-number it executes them. If there are
\* any unexecuted operations left then it sends the primary
\* a PrepareOk message with the op number it received from 
\* the primary.
...
ReceiveSV ==
    \E m \in DOMAIN messages, r \in replicas :
        ...
        /\ rep_status' = [rep_status EXCEPT ![r] = Normal]
        /\ rep_view_number' = [rep_view_number EXCEPT ![r] = m.view_number]
        /\ ApplyCheckpoint(r, m.log_suffix, m.checkpoint, m.cp_number,
...
\* Crash
\* 
\* A replica crashes and restarts with a subset of its
\* log and associated app_state. It changes its state
\* to Recovering and chooses a peer non-deterministically
\* and sends a GetCheckpoint message to it.
...
Crash ==
    ...
    \* mutations to state
    /\ \E r \in replicas :
        \E last_checkpoint_number \in 0..rep_commit_number[r] :
            /\ rep_status' = [rep_status EXCEPT ![r] = Recovering]
            /\ rep_log' = [rep_log EXCEPT ![r] = EmptyLog(last_checkpoint_number)]
            /\ rep_app_state' = [rep_app_state EXCEPT ![r] = Checkpoint(r, last_checkpoint_number)]
            /\ rep_view_number' = [rep_view_number EXCEPT ![r] = 0]
            /\ rep_op_number' = [rep_op_number EXCEPT ![r] = last_checkpoint_number]
            /\ rep_commit_number' = [rep_commit_number EXCEPT ![r] = last_checkpoint_number]
            ...
            /\ SendOnce([type      |-> GetCheckpointMsg,
                         dest      |-> AnyDest,
                         source    |-> r])
...
\* 
\* A replica receives a GetCheckpointMsg and
\* sends a NewCheckpointMsg back containing the
\* latest checkpoint number and the checkpoint itself.
...
ReceiveGetCheckpointMsg ==
    \E m \in DOMAIN messages, r \in replicas :
        ...
        /\ CanProgress(r)
        /\ rep_status[r] # Recovering
        /\ ReceivableMsg(m, GetCheckpointMsg, r)
        \* mutations to state
        /\ \E last_checkpoint_number \in 0..rep_commit_number[r] :
            DiscardAndSend(m, [type       |-> NewCheckpointMsg,
                               cp_number  |-> last_checkpoint_number,
                               checkpoint |-> LogPrefix(r, 
                                                   rep_app_state[r], 
...
\*****************************************
\* ReceiveNewCheckpointMsg
\* 
\* A replica receives a NewCheckpointMsg which includes
\* an entire checkpoint. The paper discusses the use of
\* merkle trees but this specification simply sends the
\* entire application state. 
\* The replica replaces its application state with the
\* checkpoint and deletes its log. It's existing log
\* may be divergent from the source replica and this
\* checkpointing sub-protocol does not provide a log
\* synchronization mechanism that covers the log prefix
\* which is covered by the checkpoint itself.
\* The replica now broadcasts a Recovery message including
\* the cp number of its checkpoint.
...
ReceiveNewCheckpointMsg ==
    \E m \in DOMAIN messages, r \in replicas :
        ...
        /\ CanProgress(r)
        /\ rep_status[r] = Recovering
        /\ ReceivableMsg(m, NewCheckpointMsg, r)
        \* mutations to state
        /\ rep_log' = [rep_log EXCEPT ![r] = [op \in 1..m.cp_number |-> NoLogEntry]]
        /\ rep_app_state' = [rep_app_state EXCEPT ![r] = m.checkpoint]
        /\ rep_op_number' = [rep_op_number EXCEPT ![r] = m.cp_number]
        /\ rep_commit_number' = [rep_commit_number EXCEPT ![r] = m.cp_number]
        /\ DiscardAndBroadcast(m, 
                     [type      |-> RecoveryMsg,
                      x         |-> UniqueNumber,
                      op_number |-> m.cp_number,
                      dest      |-> Nil,
                      source    |-> r], r)
        /\ UNCHANGED << rep_status, rep_view_number, rep_peer_op_number,
                        rep_last_normal_view, rep_rec_vars,
...
ReceiveRecoveryMsg ==
    \E m \in DOMAIN messages, r \in replicas :
        ...
        /\ rep_status[r] = Normal
        \* mutations to state
        /\ LET msg == [type          |-> RecoveryResponseMsg,
                       view_number   |-> View(r),
                       x             |-> m.x,
                       op_number     |-> rep_op_number[r],
                       dest          |-> m.source,
                       source        |-> r]
           IN CASE IsNormalBackup(r) -> 
                        ReplyWithNilLogSuffix(r, m, msg)
                [] Len(rep_log[r]) > m.op_number /\ rep_log[r][m.op_number+1] = NoLogEntry -> 
                        ReplyWithCheckpointAndLogSuffix(r, m, msg) 
                [] OTHER ->
                        ReplyWithLogSuffix(r, m, msg)
        /\ UNCHANGED << rep_state_vars, rep_rec_vars, rep_vc_vars,
...
\* from a majority of its peers which includes a primary
\* of the highest view among the responses.
\* It completes the recovery procedure by choosing a response
\* from a primary in the highest view and updating its state.
\* The chosen response may take two forms:
\*  - If the response only contains a log suffix, it overwrites
\*    /appends the log entries to its log and executes any operations
\*    from the commit-number advancing (which it might not).
\*  - If the response contains a checkpoint it writes the checkpoint 
\*    to its app state and overwrites/appends the log entries to its log.
\*    It also executes any ops between the received cp-number and 
\*    commit-number. 
\* It also updates its, op-number, commit-number, view and last normal view and 
\* switches back to Normal status.
        ...
        /\ \E m \in rep_rec_recv[r] : 
            ...
            /\ IF m.flag = 0
               THEN LET log == [op \in 1..m.op_number |->
                                    IF op < m.first_op
                                    THEN rep_log[r][op]    \* keep operations in replica log prefix
                                    ELSE m.log_suffix[op]] \* take operations in message log suffix
                    IN /\ rep_log' = [rep_log EXCEPT ![r] = log]
                       /\ MaybeExecuteOps(r, log, rep_commit_number[r], m.commit_number)
                       /\ rep_op_number' = [rep_op_number EXCEPT ![r] = m.op_number]
               ELSE ApplyCheckpoint(r, m.log_suffix, m.checkpoint, m.cp_number,
                                    m.op_number, m.commit_number)
            /\ rep_status' = [rep_status EXCEPT ![r] = Normal]
            /\ rep_view_number' = [rep_view_number EXCEPT ![r] = m.view_number]
            /\ rep_last_normal_view' = [rep_last_normal_view EXCEPT ![r] = m.view_number]
...
OpOf(r, op_number) ==
    IF rep_log[r][op_number] = NoLogEntry
    THEN rep_app_state[r][op_number]
    ELSE rep_log[r][op_number]

\* INV: NoLogDivergence
\* Because the log prefix might be NoOps, we also
\* check the app state.
NoLogDivergence ==
    \A op_number \in 1..Cardinality(Values) :
        ~\E r1, r2 \in replicas :
            ...
            /\ OpOf(r1, op_number) # OpOf(r2, op_number)
    ...
    \A op_number \in 1..Cardinality(Values) :
        ~\E r1, r2 \in replicas :
            ...
            /\ \/ rep_app_state[r1][op_number] # rep_app_state[r2][op_number]
               \/ rep_app_state[r1][op_number] = NoLogEntry \* would indicate a bug in the spec         
            
\* INV: AcknowledgedWritesExistOnMajority
ReplicaHasOp(r, v) ==
    \E op_number \in DOMAIN rep_log[r] :
        OpOf(r, op_number).operation = v
...
CommitNumberMatchesAppState ==
    \A r \in replicas :
        Len(rep_app_state[r]) = rep_commit_number[r]
