...
1. the "rep_" variables are a function, which programmers know as a map.
   ...
   For example, rep_status might be r1=Normal, r2=ViewChange, r3=Normal.
...
2. When you see EXCEPT, you are seeing one entry in a map being updated.
   ...
   like rep_status[r] = ViewChange in Python.
...
VARIABLES rep_status,               \* replica status (Normal or ViewChange)
          ...
          rep_app_state,            \* the replica application state (as a log)
...
\* There is no pair of replicas that both applied the
\* same op number to their application state but whose
\* value was different. Also, no replica has an inconsistency
\* between their log and their app state.
