...
- State Transfer
- Application state
- Basic replica recovery without optimizations
...
LogSuffix(r, log, op_number) ==
    ...
    THEN <<>>
    ELSE [op \in op_number+1..Len(log) |-> log[op]]
...
ReceiveGetState ==
    \E r \in replicas, m \in DOMAIN messages :
        ...
        /\ DiscardAndSend(m, 
                [type          |-> NewStateMsg,
                 view_number   |-> View(r),
                 log           |-> LogSuffix(r, rep_log[r], m.op_number),
...
ReceiveRecoveryMsg ==
    \E m \in DOMAIN messages, r \in replicas :
        ...
        /\ DiscardAndSend(m, [type          |-> RecoveryResponseMsg,
                              view_number   |-> View(r),
                              x             |-> m.x,
                              log           |-> IF IsNormalPrimary(r) 
                                                THEN rep_log[r] ELSE Nil,
                              op_number     |-> IF IsNormalPrimary(r)
                                                THEN rep_op_number[r] ELSE Nil,
                              commit_number |-> IF IsNormalPrimary(r)
...
CompleteRecovery ==
    \E r \in replicas :
        ...
        /\ \E m \in rep_rec_recv[r] :
            ...
            /\ rep_log' = [rep_log EXCEPT ![r] = m.log]
            /\ rep_op_number' = [rep_op_number EXCEPT ![r] = m.op_number]
            /\ MaybeExecuteOps(r, m.log, 0, m.commit_number)
            /\ rep_rec_recv' = [rep_rec_recv EXCEPT ![r] = {}]
...
\* RetryRecovery
\*
\* If a replica initiates recovery as a view change occurs
\* it is possible for some of its peers to receive the recovery
\* message before the view change and some after the view change
\* such that none of the responses include the primary in the
\* new view. This requires a new recovery message to be sent
\* else recovery gets stuck.
RetryRecovery ==
    \E r \in replicas :
        \* enabling conditions
        /\ CanProgress(r)
        /\ rep_status[r] = Recovering
        /\ Cardinality(rep_rec_recv[r]) > ReplicaCount \div 2
        \* Hint: There exists no response from a primary in the highest view
        /\ ~\E m \in rep_rec_recv[r] : 
            /\ m.log # Nil
            /\ ~\E m1 \in rep_rec_recv[r] :
                m1.view_number > m.view_number
        \* Hint: There is no chance of receiving another RecoveryResponse 
        /\ ~\E m \in DOMAIN messages :
            /\ \/ /\ m.type = RecoveryMsg
                  /\ CanProgress(m.dest)
               \/ m.type = RecoveryResponseMsg
            /\ m.x = rep_rec_number[r]
            /\ messages[m] > 0
        \* mutations to state
        /\ rep_rec_recv' = [rep_rec_recv EXCEPT ![r] = {}]
        /\ rep_rec_number' = [rep_rec_number EXCEPT ![r] = UniqueNumber]
        /\ Broadcast([type      |-> RecoveryMsg,
                      ...
                      dest      |-> Nil,
                      source    |-> r], r)
        /\ UNCHANGED << rep_state_vars, rep_vc_vars, aux_vars, no_prog_vars, replicas >>
