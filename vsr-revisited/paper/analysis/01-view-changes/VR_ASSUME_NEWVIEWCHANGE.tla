-------------------------------- MODULE VR_ASSUME_NEWVIEWCHANGE --------------------------------
...
- Assume-mode view numbers are incremented on the timer, but are updated
  to match the view number of an SVC or DVC message that has a higher
  view number than the replica
- Start new StartViewChange messages if view change has stalled in the SVC phase.
